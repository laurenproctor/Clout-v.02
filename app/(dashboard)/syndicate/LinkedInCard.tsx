    ...
    <div className="rounded-xl border border-zinc-200 bg-white overflow-hidden">
      ...
      <div className="flex items-center justify-between px-5 pt-4 pb-3">
        <div className="flex items-center gap-2.5">
          ...
          <span className="text-[11px] text-zinc-400">Professional · authority-driven</span>
        </div>
        <span className="text-[11px] text-zinc-400">{estimateDwellTime(content)}</span>
      </div>

      {/* Post body */}
      <div className="px-5 pb-6 cursor-pointer" onClick={onFocus}>
        <p className="text-[15px] leading-[1.75] text-zinc-800 whitespace-pre-wrap">
          {displayContent}
        ...
        {showInsights && (
          <div className="px-5 pb-5 space-y-4 border-t border-zinc-50">
            {intelligence.authority_style && (
              <div>
                <p className="text-[9px] uppercase tracking-[0.12em] text-zinc-300 mb-1.5">Authority dynamics</p>
                <p className="text-[11px] text-zinc-500 leading-relaxed">{intelligence.authority_style}</p>
              </div>
            )}
            {intelligence.emotional_style && (
              <div>
                <p className="text-[9px] uppercase tracking-[0.12em] text-zinc-300 mb-1.5">Emotional register</p>
            ...
            {intelligence.persuasive_mechanics.length > 0 && (
              <div>
                <p className="text-[9px] uppercase tracking-[0.12em] text-zinc-300 mb-1.5">Persuasive mechanics</p>
                <div className="space-y-1">
                  {intelligence.persuasive_mechanics.slice(0, 3).map((m, i) => (
                    <p key={i} className="text-[11px] text-zinc-500 leading-relaxed">· {m}</p>
                  ))}
                </div>
              </div>
            )}
            <div>
              <p className="text-[9px] uppercase tracking-[0.12em] text-zinc-300 mb-1.5">Audience</p>
              <p className="text-[11px] text-zinc-400 leading-relaxed">{truncateAtWord(intelligence.audience, 80)}</p>
            </div>
            {intelligence.platform_risks?.linkedin && (
              <div>
                ...
                <p className="text-[11px] text-zinc-400 leading-relaxed">{intelligence.platform_risks.linkedin}</p>
              </div>
            )}
          </div>
        )}
