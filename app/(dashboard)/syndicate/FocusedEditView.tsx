    ...
    <div className="space-y-4">
      ...
      <div className="rounded-lg border border-zinc-200 p-5 space-y-4">
        <div>
          ...
          <p className="text-sm text-zinc-400 mt-0.5">{PLATFORM_DESCRIPTORS[platform]}</p>
        </div>
        <textarea
          value={content}
        ...
        <div className="flex flex-wrap gap-2">
          <button onClick={handleCopy} className="rounded-md border border-zinc-200 px-3 py-1.5 text-sm font-medium text-zinc-700 hover:bg-zinc-50 transition-colors">
            {copied ? 'Copied' : 'Copy'}
          </button>
          <button onClick={onRegenerate} className="rounded-md border border-zinc-200 px-3 py-1.5 text-sm font-medium text-zinc-700 hover:bg-zinc-50 transition-colors">
