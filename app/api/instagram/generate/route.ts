...
export async function POST(req: NextRequest) {
  ...
  const request = (body as { request?: unknown }).request as InstagramGenerationRequest

  if (!request?.visualFormat || !request?.sourceContent || !request?.intent || !request?.audience) {
    return new Response(
      JSON.stringify({ error: 'visualFormat, sourceContent, intent, and audience are required' }),
      { status: 400 }
    )
  }
  ...
  if (request.sourceType === 'url' || looksLikeUrl(request.sourceContent)) {
    ...
    try {
      ...
      console.warn('[instagram/generate] URL scrape failed, proceeding with URL as context:', msg)
