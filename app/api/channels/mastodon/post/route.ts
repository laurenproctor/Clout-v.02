...
import { publishMastodonOutput, acquirePublishLock, markPublished, markFailed } from '@/lib/domain/publishing'
...
export async function POST(req: NextRequest) {
  ...
  try {
    const { postId, postUrl } = await publishMastodonOutput(output)
    await markPublished(outputId, postId, postUrl)
