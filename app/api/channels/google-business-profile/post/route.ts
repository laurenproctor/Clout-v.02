...
import { publishGBPOutput, acquirePublishLock, markPublished, markFailed } from '@/lib/domain/publishing'
...
export async function POST(req: NextRequest) {
  ...
  if (output.providerPostId) {
    return NextResponse.json({ ok: true, postName: output.providerPostId, alreadyPublished: true })
  }
  ...
  try {
    const { postName } = await publishGBPOutput(output)
    await markPublished(outputId, postName)
    return NextResponse.json({ ok: true, postName })
  } catch (err) {
