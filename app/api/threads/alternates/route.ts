...
import { runThreadsAlternates } from '@/lib/threads/runGeneration'
...
// On-demand "Show alternate angles" for Threads: same brief as the anchor
// generation, plus the already-shown anchor body for genuine distinctness.
export async function POST(req: NextRequest) {
  ...
  if (request.sourceType === 'url' || looksLikeUrl(request.sourceContent)) {
    ...
    try {
      ...
      console.warn('[threads/alternates] URL scrape failed, proceeding with URL as context:', msg)
  ...
  const stream = runThreadsAlternates({ request, lenses: resolvedLenses, brandContext, campaignContext }, anchorBody)
