...
export function GoogleSearchConsoleCard() {
  ...
  const [state, setState] = useState<GSCState>({ connected: false, sites: [], selectedSiteUrl: null, summary: null })
  ...
  const load = useCallback(async () => {
    try {
      const res = await fetch('/api/integrations/google/search-console')
          ...
          <div className="pt-0.5">
            ...
            <p className="mt-0.5 text-xs text-zinc-500">
              {connected && selectedSiteUrl
                ...
                : 'Organic search impressions, clicks, and keyword rankings'}
            </p>
          </div>
        </div>
