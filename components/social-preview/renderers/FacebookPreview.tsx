...
export function FacebookPreview({ data, theme, density }: RendererProps) {
    ...
    >
      <div style={{ padding: isMini ? 10 : '12px 16px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <div style={{ minWidth: 0 }}>
            ...
            <div style={{ fontSize: 12, color: p.muted }}>{data.timestamp ?? 'Just now'} · 🌐</div>
          </div>
        </div>
      ...
      {data.carousel ? (
        <PreviewCarouselBlock
          ...
          palette={p}
          chrome="flat"
          aspectRatio={data.carousel.images?.[0]?.aspectRatio ?? spec.defaultRatio}
        />
      ) : media ? (
        <PreviewMediaBlock
          media={media}
      ...
      {!isMini && (
        <div style={{ padding: '4px 16px 10px' }}>
          <PreviewActionBar
            actions={['thumbsup', 'comment', 'share']}
            density={density}
            color={p.muted}
            withLabels
            spread
