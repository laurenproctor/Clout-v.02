...
export function SubjectsInput({ value, onChange, className }: SubjectsInputProps) {
    ...
    <div className={cn('flex flex-col gap-1.5', className)}>
      ...
      >
        ...
        <input
          ...
          onKeyDown={handleKey}
          onBlur={add}
          placeholder={value.length === 0 ? 'e.g. People at work, Urban streets, Close-up products...' : ''}
          className="min-w-[120px] flex-1 bg-transparent text-sm outline-none placeholder:text-zinc-400"
        />
      </div>
      <p className="text-xs text-zinc-400">Press Enter or comma to add. Describe scenes, subjects, and settings to include.</p>
    </div>
  )
}
